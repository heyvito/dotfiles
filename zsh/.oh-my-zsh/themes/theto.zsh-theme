...
prompt_vi_git_status() {
  if git rev-parse --git-dir > /dev/null 2>&1; then
    ...
    echo -n "%{$fg[black]%}$box_prefix%{$fg[white]%}%{$bg[black]%}\uf418 $(prompt_vi_git_info) %{$reset_color%}%{$bg[$status_color]%} %{$reset_color%}%{$fg[$status_color]%}$box_suffix%{$reset_color%}"
  fi
}
...
CURRENT_LOCATION="%{$fg[black]%}$box_prefix%{$fg[white]%}%{$bg[black]%}%1~%{$reset_color%}%{$fg[black]%}$box_suffix%{$reset_color%}"
